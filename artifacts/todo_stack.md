...
- **23-COMPAT-JOM-01** — JaxFNE 0.4.22 compatibility regressions (downstream evidence; not fact_stack): independently reproduce `resolve_edge_delay_steps` / `np.asarray` tracer unsafety and `tau_ms` shape `(0,)` validation regression; classify each `JAXFNE_REGRESSION` | `DOWNSTREAM_INVALID_USAGE` | `NOT_REPRODUCED`; if `JAXFNE_REGRESSION` apply smallest generic repair preserving 0.4.22 intended semantics with focused tests (JIT/tracer, empty/placeholder edge storage, delay/tau materialization, no scientific trajectory change outside corrected behavior); no HDP features; acceptance: JaxFNE regression probes pass, Jomission migration suite rerunnable against repaired engine, exact behavior/provenance reported
