...
$$\frac{dv_i}{dt}=0.04v_i^2+5v_i+140-u_i+I_i^{native}(t), \qquad \frac{du_i}{dt}=a_i(b_i v_i-u_i)$$

Reset rule:

$$v_i\ge 30 \Rightarrow v_i\leftarrow c_i,\quad u_i\leftarrow u_i+d_i$$

Terms: $v_i$ is the reduced voltage-like state, $u_i$ is recovery, $a,b,c,d$ set class-specific dynamics, and $I_i^{native}$ combines base drive, recurrent input, and native stochastic drive.

Worded equation: voltage changes according to intrinsic recovery, a nonlinear voltage term, and native input; threshold events reset the emitter state.
...
Failure modes: unstable native drive, high synchrony, insufficient duration for low-frequency spectra, and oversized dense recurrent matrices on small CPUs.
...
This tutorial covers reduced emitters, declared source projection, relative proxy readouts, and package-native figure generation. Solver-calibrated amplitudes, subject-specific head geometry, and empirical parameter fitting belong to later workflows.
