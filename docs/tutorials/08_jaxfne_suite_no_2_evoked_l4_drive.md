...
Run the tutorial notebook to generate these figures.

## Interpretation

### Baseline firing rate

Baseline (no stimulus): V1 column shows low, irregular firing from background tonic input. L4 fires sparsely; L2/3 and L5 show modulated bursting from recurrent feedback.

### Evoked response onset latency

L4 drive raises L4 firing within 50–100 ms of onset (direct feedforward response). L2/3 responds later (~100–200 ms) via synaptic transmission and local routing.

### Amplitude modulation

Evoked rate rises with drive amplitude. Higher L4 drive does not monotonically raise L2/3 response due to refractory period and adaptation in the reduced Izhikevich model.

### Field readout polarity

LFP-proxy shows a depolarizing L4 sink during the stimulus (inward synaptic current; negative CSD-proxy = current sink = inward current, per the convention `csd_proxy = -d²φ/dz²`).

### Post-stimulus suppression

After offset, some neurons show brief rebound burst (adaptation-current deinactivation), then return to baseline. This rebound is a proxy artifact, not biological post-stimulus adaptation.
