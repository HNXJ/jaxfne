...
| 4 Biological/physical boundaries | L1 proxy boundary; HDP relative/latent state; clamps = finiteness not stability; K_w_ctrl = 0.0 horizon; B1 compatibility not validation; quarantined generic LFP question (no % claimed) | Para 5 ("Every readout reported here is a relative computational proxy"; "linear projection/proxy operators, not volume-conductor or partial-differential-equation solves"; "implementation-bounded by hard clamps that guarantee finiteness, which is not dynamical stability"; "no active restoration to connection magnitudes"; "compatibility is not biological validation"; "How much spike-associated versus slower transmembrane currents contribute to the modeled field under specified geometry and filtering remains a generic quantitative question; no percentage is claimed here") | Methods L6/5.3; CL-03,04; A-3 receipt; Results HDP paragraphs; `artifacts/publication/publication_evidence_index.json` (no percentage claimed; quarantine boundary) |
...
| 6 Future directions | F1-F6 downstream of limitations; wave-positive regime with u ∈ X; H_clock vs H_excit recovery-clock discriminator (T0 vs τ_u tracking with ρ_volley/CV_T/rate guard, UNRESOLVED_MECHANISM); developmental operator = future possibility only, unnamed | Para 8 ("Calibrated source bridges"; "validated field solvers"; "gradient-based inverse problems"; "Broader dynamical families"; "wave-positive regime search" + "recovery (with u ∈ X, not H)" + "A preregistered recovery-clock battery further distinguishes H_clock (period T0 tracks recovery timescale τ_u=1/a_E with slope within 30% and Spearman ρ≥0.40, regime preserved: ρ_volley within 0.20, CV_T≤0.35, rates within 2×) from H_excit (T0 shift via excitability/rate change, regime collapse → UNRESOLVED_MECHANISM); this battery is deferred to a separately staged execution (e2_c_recovery_clock_prereg.v2, spec_hash b7840069) and is not performed here" + "a developmental operator could generate a realized circuit specification upstream of the existing pipeline; this is a future possibility, not a capability of the present work") | L1, L8, L2, L5, L2 + `e2_c_recovery_clock_prereg.v2.json:499-583` H_clock_vs_H_excit; `S_PULSE_CHARACTERIZATION.md` f0 7.22 Hz context |
