...
where X are neural state trajectories (including membrane potential v, recovery variable u, spikes and synaptic state; u ∈ X is the emitter-state recovery coordinate with du/dt = a(bv−u) and spike reset v→c, u→u+d, distinct from RBS), H hidden (Relative Biophysical State, RBS) coordinates (finite-dimensional container H ∈ R^{d_H} whose influence on E,S,F,P requires typed coupling maps Γ_H; RBS is not intrinsically homeostasis and u is not an H coordinate), S the source operator, Q the canonical relative source, F the field operator, Φ the field-time proxy, P the probe operator, and Y the final readout. In operator terms:

- **S_ψ**: `x_t ↦ q_t` — the source operator maps the emitter state to the canonical relative source Q at each time step (layer 3, Eq. 4).
- **E_θ**: `(x_t, u_t, ξ_t) ↦ x_{t+1}` — the evolution operator (with u_t ∈ x_t as the recovery coordinate within X: x_t = (v_t, u_t, synaptic_state_t, s_t)); for HDP runs the hidden coordinate H becomes part of the carried state (layer 5).
...
Protocols D, E, and W operate on typed hidden coordinates H (per-neuron, Relative). The Izhikevich recovery variable u is not an H coordinate: u ∈ X with dynamics Eq. 2 and reset u→u+d, while H ∈ R^{d_H} is a separate typed container; H influences the emitter only through Γ_H (e.g., b_eff = H_K·b). Two families are used:
...
A-3 executed both presets × seeds {1001, 1002, 1003} on the frozen C3 ring (duration 2000 ms, Δt 0.5 ms, drive as C3, K_w_ctrl = 0.0) and recorded extremal invariants per run (H range, |w| growth factor, |v| ≤ 150, |u| ≤ 2000, |syn| ≤ 1e4); no parameter was tuned and no instability inference made. Observed bounds (Relative): DEFAULT_HDP H ∈ [1.0000, 1.0008], w = 6.0 unchanging (growth 1.00000); DEFAULT_HDP_DESYNC H ∈ [1.0000, 1.0310], w ∈ [5.87, 6.17] (growth 1.00004). All reported statements are scoped to the tested domain ("trajectories remained bounded over the tested parameter and time domain"); no claim of unbounded divergence, and no claim of stability beyond the tested domain.
...
All wave classifications in the C-series (60 cells) and A-series use the single estimator `estimate_traveling_wave` (module SHA 684859a…; late revision disclosure below), configured exactly with the preregistered parameters: band [8.0, 13.0] Hz, minimum spatial coherence 0.55, R²-phase-traveling threshold 0.6, coherence-traveling threshold 0.55, noise band-power floor 1e-4. Pipeline per condition:
...
- **Measured performance (Relative error envelope at the tested grid):** ε_f = 0 across cases; ε_k̂ ≤ 0.00831 (relative), ε_θ = 0, ε_v ≤ 0.00824; the only sensitivity loss appears at amplitude A = 0.05 with σ = 0.5 (2/40 cases in the A-2 S1 grid below). A-1a validates the estimator in isolation; the dynamical question of whether the network generates waves is A-1b.
...
- **S1 — amplitude × noise grid:** 40 = 5×4×2 cases over A ∈ {0.05, 0.1, 0.2, 0.5, 1.0} × σ ∈ {0, 0.1, 0.25, 0.5} × m ∈ {1, 2} (f = 10 Hz, sign +, φ₀ chosen), recording which gate fails first (band-power floor, k̂ < k_min, stability, coherence/R²). Result: all cases with amplitude ≥ 0.05 and σ ≤ 0.25 detected ("all 38 with amplitude ≥ 0.05 and σ ≤ 0.25 were detected; only two failures at A = 0.05, σ = 0.5").
- **S2 — C3-regime embedding:** for 3 representative frozen NO_WAVE C3 cells (condition ordered_uniform; seeds 1001, 1002, 1009, covering all three frozen NO_WAVE reason classes: standing/flipping, synchronous, structured-but-fails), embed a known traveling wave: `Φ_mix = Φ_c3_bp + γ·rms_c3·cos(mx − ωt + φ₀)` with rms_c3 = √mean(Φ_c3_bandpassed²) computed from the cell's own frozen V_m before embedding, γ ∈ lattice up to 4.0, φ₀ ∈ {0.0, 1.57} (36 = 3×6×2 cases). Flip threshold γ* = smallest γ with TRAVELING_WAVE; ceiling reported as NO_FLIP_WITHIN_LATTICE. Result: γ* = 1.0 at both φ₀ for all 3 cells — a genuine traveling component of in-band RMS amplitude is not hidden by the frozen C3 field structure.
- **S3 — duration × site count:** clean m = 1, f = 10 Hz wave over duration {250, 500, 1000, 2000} ms × sites {24, 12, 6} (uniform stride subsampling) = 12 cases; classification reliability floor reported (NO_WAVE at 6 sites for 500 and 2000 ms durations, standing-gate parity limitation). Note on frequency resolution: the rfft bin spacing is fs/N = 2000/duration (0.5 Hz at full duration, 4 Hz at 250 ms), so at short durations the frequency estimate is quantized (e.g. 2 Hz off-bin at 250 ms) while the coherence/R² gates still classify correctly; frequency-error reporting in the S3 stage reflects this bin quantization, not estimator noise.

## 10. Dynamic search for a positive regime (A-1b)

- **Design:** 15 = 5×3 lattice points over v_c ∈ {0.033, 0.065, 0.131, 0.262, 0.524} mm/ms × K ∈ {1, 2, 4}; × 3 seeds {1001, 1002, 1003} = 45 cells; delays from Eq. 6; all else identical to frozen C3 ordered conditions (same construction, same drive n0 pulse A = 50 at 200–201 ms and n6 pulse A = 45 at 800–801 ms, duration 2000 ms, Δt 0.5 ms, σ_n = 0.0, enable_hdp false).
- **Anchor:** v_c = 0.131, K = 1 must reproduce the frozen C3 ordered-uniform edges and emitter bitwise (construction identity) and the V_m traces bitwise; any failure relabels the point as a nearby control with anchor flag false (criteria/outcomes unchanged). Verified: 3/3 seeds, max |ΔV_m| = 0.0.
...
- **Arm-isolation gates:** E5 G1 (N1 and D share H_K trajectories on owners; only Γ_H differs) — verified bit-exact H_K(N1) == H_K(D) 3/3 seeds; G2 N0 ≡ N1 at H_K = 1 (bit-exact V_m/spikes/Q); G3 owner contrast measured per seed with declared metrics; all G1–G10 pass; single source of truth (one simulate per arm/seed).
