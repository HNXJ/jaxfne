...
invariant hash(Q_Fig2) = hash(Q_Fig3) = hash(Q_Fig4). This is a deterministic
...
of that same frozen source Q, not experimental recordings. The toy EEG/MEG leadfields
...
and the registered classification record are unchanged since original registration,
...
mm/s — the estimator recovered all 48, with zero frequency error,
wave-number error bounded by 0.00831 relative, zero direction error, and
phase-velocity error bounded by 0.00824 relative. Five synthetic negative controls —
synchronous oscillation, standing wave, random spatial phases, noise-only, and a true
travelling plane with shuffled sensor coordinates — were all correctly rejected
.
...
2000 ms at Δt = 0.5 ms. All 60 cells were classified NO_WAVE, for three
reasons: 52 synchronous oscillations with near-zero wave number, 4 standing/flipping
spatial gradients, 4 structured-but-fails traveling gates. A scope qualifier applies:
...
specification; each point was run on 3 seeds (45 cells total). The anchor point
(vc0.131_k1) is bitwise identical to the frozen ring for all 3 seeds (max |ΔV_m| = 0.0). All 45 cells were classified NO_WAVE with 0 invalid cells, yielding the
predeclared outcome NO_POSITIVE_DOMAIN_IN_TESTED_RANGE; failure reasons mirror the
grid (27 standing/flipping, 11 synchronous, 7 structured-but-fails). Because only 2
...
tested amplitude/noise cases (amplitudes 0.05–1.0, noise σ 0–0.5), all 38 with
...
initializations. Across 12 duration/site cases, detection is reliable for ≥ 12
...
timestep discretization to the same four-step delay as the uniform condition.
...
predeclared cases. Activity in this search was sparse, with only 2 of 24
neurons spiking per cell at rates ≤0.5 Hz, consistent with a regime in which
local activity was not regenerated as a propagating population mode. Together with the
60-cell grid, which was dominated by synchronous states (52/60 synchronous), these
...
uniform), 0.0 (long, heterogeneous); factorial estimates are α_length = 0.0,
α_heterogeneity = +0.0521, α_interaction = −0.0521. The interaction cancels the
...
Across 36 cells × 4 arms (N0, N1, N2, D; 3 seeds × 3 recovery intervals), the D arm
shows observable spike-count attenuation with A_adapt > 0.15 in 9 of 9 cells; formal
...
1 + θ_H) passes 0/9. The all-arms classification counts are N0/N1/N2: 9 ADAPTATION
each; D: 9 NO_ADAPTATION; the mechanism gates are evaluated where H_K is written (the
...
null on A_adapt, 0.2857 vs 0.2857); therefore attenuation is not attributable to
...
Across the frozen W3b lattice of 2187 points, 243 are classified D (dormant/vanishing
feedback), 0 robustly stable S, 0 near-critical C, 0 unstable U, and 1944 remain active but
stability-unresolved X. With N_S = 0, no robustly stable active point was classified, and no useful robust-active hidden-state dynamics (HDP) domain is claimed; the stability classification of
...
the DESYNC preset H ∈ [1.0000, 1.0310], |w| ∈ [5.8692, 6.1684], growth ratio 1.00004.
...
defaults; K_w_ctrl = 0.0). These trajectories remained bounded over the tested
...
negative control of the propagation result . When the same local H_K
perturbation is expressed through its typed Γ_H coupling, the E5 protocol — 3 arms
(N0/N1/D) × 3 seeds = 9 trajectories — passes its sanity gates (N0 ≡ N1 bit-exactly
on V_m, spikes and Q across all 3 seeds; H_K identical between the N1 and D arms) and classifies every seed as HIERARCHICAL_PROPAGATION (3/3). The perturbation
...
the other area (A1) 3.16 mV with 9 additional spikes. Propagation here is structural
...
match the shipped figure bytes (7/7). The recorded equivalence gate confirms byte
equality for 7/7 figure cases across the frozen and re-rendered artifacts. The figure
regeneration path is reproducible from a clean checkout per the documented entrypoint, with
pinned dependencies (matplotlib ≥ 3.10.9, < 3.11; scipy == 1.17.1). Byte-for-byte
...
markers are verified by `scripts/audit_results_draft.py`.
