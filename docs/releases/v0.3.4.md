...
jaxfne v0.3.4 marks the stable v0.3 tutorial-scenario release. The chainable Configuration grammar is validated (schema-accepted and executable end-to-end, not an empirical/physical claim), and the core tutorials are executable with proper optional dependency handling.

**Key theme:** v0.3.4 is "tutorial-scenario spine with validated public grammar" — validated here means the grammar runs and round-trips correctly, not that any simulated output has been empirically confirmed.
...
Three v0.3.x core tutorials execute end-to-end and produce finite, schema-valid output (execution validation, not empirical/physical validation):
...
v0.3.4 is a stable checkpoint. Reserved v0.3.x work can focus on:
