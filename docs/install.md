...
previous release is **`0.4.22`** (tag `v0.4.22`). The `dev` source tree is
currently the **`0.4.24` release candidate** (not yet published to PyPI;
`jaxfne.__version__` reads `0.4.24` from source). The current **development**
