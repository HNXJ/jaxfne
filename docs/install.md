...
The current release is **`jaxfne==0.4.4`** (tag `v0.4.4`), published to PyPI as
both a wheel (`jaxfne-0.4.4-py3-none-any.whl`) and an sdist
(`jaxfne-0.4.4.tar.gz`). To pin the exact release used by the repository
tutorials:
