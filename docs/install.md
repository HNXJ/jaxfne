...
The current release is **`jaxfne==0.4.3`** (tag `v0.4.3`), published to PyPI as
both a wheel (`jaxfne-0.4.3-py3-none-any.whl`) and an sdist
(`jaxfne-0.4.3.tar.gz`). To pin the exact release used by the repository
tutorials:

```bash
pip install "jaxfne==0.4.3"
```
