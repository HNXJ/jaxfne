...
The current release is **`jaxfne==0.4.1`** (tag `v0.4.1`), published to PyPI as
both a wheel (`jaxfne-0.4.1-py3-none-any.whl`) and an sdist
(`jaxfne-0.4.1.tar.gz`). To pin the exact release used by the repository
tutorials:

```bash
pip install "jaxfne==0.4.1"
```
