...
- [ ] Import smoke: `python -c "import jaxfne; print(jaxfne.__version__)"` — confirms 0.3.22
...
- [ ] Package build: `python -m build` — generates `dist/jaxfne-0.3.22.tar.gz` and `dist/jaxfne-0.3.22-py3-none-any.whl`
...
- [ ] TestPyPI install smoke: `pip install --index-url https://test.pypi.org/simple/ jaxfne==0.3.22 && python -c "import jaxfne; print(jaxfne.__version__)"`
...
- [ ] Tag provenance verified: `git cat-file -t v0.3.22`, `git branch --contains v0.3.22`, `git ls-remote --tags origin v0.3.22`
- [ ] Create annotated release tag on `main`: `git tag -a v0.3.22 -m "Release v0.3.22: Etude No. 1, template standardization, hygiene rule clarifications"` (pending authorization)
- [ ] Push tag to remote: `git push origin v0.3.22` (pending authorization)
- [ ] GitHub release created from tag: Visit https://github.com/HNXJ/jaxfne/releases/new?tag=v0.3.22

### Release Metadata
- [ ] CHANGELOG.md updated with v0.3.22 entry ✅
- [ ] docs/changelog.md updated with v0.3.22 entry ✅
- [ ] pyproject.toml version: 0.3.22 ✅
