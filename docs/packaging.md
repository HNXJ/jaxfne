# Packaging and Distribution (v0.3.4)
...
- **Wheel** (`jaxfne-0.3.4-py3-none-any.whl`) — binary distribution (recommended for most users)
- **Source tarball** (`jaxfne-0.3.4.tar.gz`) — source distribution (for custom builds)
...
ls -lh dist/
# Expected:
# -rw-r--r--  jaxfne-0.3.4-py3-none-any.whl  (typical: ~50KB)
# -rw-r--r--  jaxfne-0.3.4.tar.gz             (typical: ~100KB)
```

### 4. Verify Wheel Contents

```bash
python -m zipfile -l dist/jaxfne-0.3.4-py3-none-any.whl | head -30
```
...
- `jaxfne-0.3.4.dist-info/METADATA`
- `jaxfne-0.3.4.dist-info/WHEEL`
- `jaxfne-0.3.4.dist-info/RECORD`

### 5. Verify Source Distribution

```bash
tar -tzf dist/jaxfne-0.3.4.tar.gz | head -20
```

**Expected files in tarball:**
- `jaxfne-0.3.4/pyproject.toml`
- `jaxfne-0.3.4/README.md`
- `jaxfne-0.3.4/jaxfne/__init__.py`
- `jaxfne-0.3.4/tests/` (all test files)
- `jaxfne-0.3.4/examples/` (all example scripts)
- `jaxfne-0.3.4/docs/` (all documentation)
...
pip install dist/jaxfne-0.3.4-py3-none-any.whl
...
pip install dist/jaxfne-0.3.4.tar.gz
...
tar -xzf dist/jaxfne-0.3.4.tar.gz
cd jaxfne-0.3.4
...
# Upload to TestPyPI
python -m twine upload --repository testpypi dist/jaxfne-0.3.4*
```

**Verify on TestPyPI:**
```bash
# Install from TestPyPI
pip install --index-url https://test.pypi.org/simple/ jaxfne==0.3.4
...
python -m twine upload dist/jaxfne-0.3.4*
```

**Expected output:**
```
Uploading jaxfne-0.3.4-py3-none-any.whl
100% |████████████████████████████| 50KB/0.5s
Uploading jaxfne-0.3.4.tar.gz
100% |████████████████████████████| 100KB/0.3s
```

### 3. Verify on PyPI

Open https://pypi.org/project/jaxfne/0.3.4/ and verify:
- ✓ Version 0.3.4 is listed
...
```bash
ls -lh dist/
python -m zipfile -l dist/jaxfne-0.3.4-py3-none-any.whl | head -5
```
