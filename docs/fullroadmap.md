...
| B-07a | `[GATE]` `pytest tests/test_phaseB_stage1_rule_survey.py -v`; all pass | `tests/test_phaseB_stage1_rule_survey.py` | `[GATE]` `[EVIDENCE]` | `[SERIAL]` | |
...
| # | Action | File(s) | Tag | Seq | Notes |
|---|--------|---------|-----|-----|-------|
| C-01 | Read `simulate_edge_recurrent_izhikevich_hdp` in full; map every field in `final_state` / `init_state`; confirm `H` is present in both | `jaxfne/emitters.py` | `[SERIAL]` | `[SERIAL]` | |
| C-02 | Read `jaxfne/_pipeline.py`; confirm `DynamicState` has all six fields: `v, u, prev_spikes, syn_state, H, w`; if any missing: add field + test + note in `skills/FRICTIONS_STACK.md` | `jaxfne/_pipeline.py`, `skills/FRICTIONS_STACK.md` | `[SERIAL]` | `[SERIAL]` after C-01 | |
| C-02a | `[SKIP-IF]` If C-02 finds a missing field: add it to `DynamicState`; update `compile_step_fn` and `scan_network` to pass it through | `jaxfne/_pipeline.py` | `[SKIP-IF]` | `[SERIAL]` | Only if gap found |
| C-03 | Read `tests/test_sanity_delta_backup_resume.py` and `tests/test_sanity_delta_resume_equivalence_full.py`; confirm whether H carry is already gated byte-identically | `tests/test_sanity_delta_*.py` | `[SKIP-IF]` | `[PARALLEL-OK]` with C-01/C-02 | If already gated: mark C-04 `[SKIP-IF done]` |
| C-04 | Write `tests/test_phaseC_H_carry_resume.py`: (a) 2000-step one-chunk run; (b) two 1000-step chunks; assert `jnp.allclose(one_chunk_H[-1000:], two_chunk_H, atol=1e-5)`; `[NULL-CTRL]` k_gain=0 → H constant throughout both chunks | `tests/test_phaseC_H_carry_resume.py` | `[SERIAL]` | `[SERIAL]` after C-03 | Skip if C-03 found it already gated |
...
| # | Action | File(s) | Tag | Seq | Notes |
|---|--------|---------|-----|-----|-------|
| D-01 | Find the file that defines `Source` / `SourceTensor` (grep for `class Source` or `SourceTensor` in `jaxfne/`); read it in full; list every field, dtype, shape convention, `source_calibration_status` | `jaxfne/` (grep) | `[SERIAL]` | `[SERIAL]` | |
| D-02 | Read `_construct_population.py`; confirm each emitter path sets `source_calibration_status` before returning; list any path that does not | `jaxfne/_construct_population.py` | `[SERIAL]` | `[SERIAL]` after D-01 | Emitter paths: `simulate_eig_izhikevich`, `simulate_edge_recurrent_izhikevich`, `simulate_edge_recurrent_izhikevich_homeostatic`, `simulate_edge_recurrent_izhikevich_hdp`, `simulate_homeostatic_ei` |
| D-02a | `[SKIP-IF]` For each path in D-02 that does NOT set `source_calibration_status`: add the correct string assignment immediately before the return statement | `jaxfne/_construct_population.py` | `[SKIP-IF]` | `[SERIAL]` after D-02 | Only if gap found |
| D-03 | `grep -r "source_calibration_status" tests/`; list covered paths vs. uncovered | `tests/` | `[PARALLEL-OK]` with D-02 | `[SERIAL]` | |
