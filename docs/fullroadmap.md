...
## Phase D — Source Schema Stability `[BLOCKED on Phase C]`

**Goal:** Every emitter path produces a Source with a stable, documented,
tested `source_calibration_status`. Downstream Field and Probe never infer
schema from shape alone.

**Entry gate:** Phase C exit criteria met.
**Key files:** `jaxfne/` (sources definition), `jaxfne/_construct_population.py`

| # | Action | File(s) | Tag | Seq | Notes |
|---|--------|---------|-----|-----|-------|
| D-01 | Find the file that defines `Source` / `SourceTensor` (grep for `class Source` or `SourceTensor` in `jaxfne/`); read it in full; document every field, dtype, shape convention, `source_calibration_status` | `jaxfne/` (grep) | `[SERIAL]` | `[SERIAL]` | `[COMPLETED]` No `Source`/`SourceTensor` class exists anywhere in `jaxfne/`. "Source" is (a) a raw proxy current trace array, third element returned by every emitter kernel, shape `(T, N)`, float32, `source_scale * (drive + syn + noise + spike_impulse)`; and (b) metadata strings on dataclasses. Central bookkeeping: `_model.py:221` `_SOURCE_PROXY_METADATA` (`source_calibration_status="uncalibrated_izhikevich_native_current"`, `physical_amplitude_calibrated=False`). Per-dataclass defaults: `emitters.py:68` `ReceptorSpec` and `:77` `SynapseSpec` = `metadata_only_uncalibrated`; `emitters.py:120` `IzhikevichParams`; `:501` `EdgeList`; `emitters_homeostatic_ei.py:337` `HomeostaticEIParams` = `uncalibrated_homeostatic_ei_native_current`; bridges.py bridge specs = `uncalibrated_{bridge}_output` etc. `Configuration._default_metadata` (`_config.py:65-83`) carries the value and `physical_amplitude_calibrated=False`. Correcting D-01's premise: there is no class to read; the schema is the metadata + array convention above. |
| D-02 | Read `_construct_population.py`; confirm each emitter path sets `source_calibration_status` before returning; list any path that does not | `jaxfne/_construct_population.py`, `jaxfne/emitters.py`, `jaxfne/emitters_homeostatic_ei.py` | `[SERIAL]` | `[SERIAL]` after D-01 | `[COMPLETED]` Emitters themselves never write `source_calibration_status`; it lives on params objects (`IzhikevichParams:120`, `EdgeList:501`, `HomeostaticEIParams:337`, `ReceptorSpec/SynapseSpec:68/77`) as a fixed default. Every kernel returns `(voltages, spikes, sources, [diagnostics])` with `sources` an ADJUSTER-uncalibrated proxy (`simulate_eig_izhikevich:389`, `simulate_edge_recurrent_izhikevich:561`, `..._homeostatic:677`, `..._hdp:1040`, `simulate_homeostatic_ei` in `emitters_homeostatic_ei.py:462`, also `simulate_receptor_exponential_izhikevich:1661`). `toggle`-level routing: `model_simulate.py:561-571` builds consolidated `source_bookkeeping` from `_SOURCE_PROXY_METADATA` for every path (constant; identical value whichever emitter ran). HDP path (`:591`) adds `homeostasis`/`hdp` diagnostics but does not change `source_calibration_status`. No gap found: all five paths resolve to the same canonical status string. |
| D-02a | `[SKIP-IF]` not needed (no gap in D-02; status is uniform and constant) | — | `[SKIP-IF]` → `[SKIPPED]` | after D-02 | | |
| D-03 | `grep -r "source_calibration_status" tests/`; list covered paths vs. uncovered | `tests/` | `[PARALLEL-OK]` with D-02 | `[SERIAL]` | `[COMPLETED]` — Direct test coverage exists in `tests/test_source_bookkeeping_v020.py` (asserts presence, exact canonical string, `physical_amplitude_calibrated=False`, double-count guard, manifest/receipt wiring) and `tests/test_emitter_equations_v020.py` (`source_coverage` of params), `tests/test_calibration_contracts_v025.py`, `tests/test_truth_gate_clamp_v046.py`, `tests/test_manifest_v005.py`. Emitter-path-level tests (`test_homeostasis_dispatch.py`, `test_hdp_kernel_standalone.py`, `test_homeostatic_ei_*.py`) exercise the kernels but assert no `source_calibration_status`/`physical_amplitude_calibrated` on their own return values. Coverage matrix: model-level metadata/receipt/manifest = covered; per-emitter-kernel assertion of status string = uncovered (kernel-built; no per-kernel writable status exists to assert). |
| D-04 | Write `tests/test_phaseD_source_schema.py`: one test per emitter path; assert `source_calibration_status` is not None, is non-empty string, matches documented value; `[NULL-CTRL]` assert `physical_amplitude_calibrated=False` for all paths | `tests/test_phaseD_source_schema.py` | `[SERIAL]` | `[SERIAL]` after D-02a + D-03 | |
| D-05 | `[GATE]` `pytest tests/test_phaseD_source_schema.py -v`; all pass | `tests/test_phaseD_source_schema.py` | `[GATE]` `[EVIDENCE]` | `[SERIAL]` | |
| D-06 | `[DOCONLY]` Write `docs/api/source_schema.md`: table of emitter path → `source_calibration_status` value → `physical_amplitude_calibrated`; language: proxy/scaffold only | `docs/api/source_schema.md` | `[DOCONLY]` `[PARALLEL-OK]` with D-05 | `[SERIAL]` after D-04 | |
| D-07 | `[DOCONLY]` Add `source_schema.md` to `mkdocs.yml` nav under `API Reference` | `mkdocs.yml` | `[DOCONLY]` | `[SERIAL]` after D-06 | |
| D-08 | `[GATE]` `python3 -m mkdocs build --strict`; zero errors | — | `[GATE]` `[EVIDENCE]` | `[SERIAL]` last | |

### Phase D exit criteria

- D-05 + D-08 `[GATE]` green.
- Every emitter path has confirmed, tested `source_calibration_status`.
