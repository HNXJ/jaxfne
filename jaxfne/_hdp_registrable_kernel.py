"""Registered-rule HDP edge-list kernel (zero-delay path)."""
...
def simulate_edge_recurrent_izhikevich_hdp_registered(
    ...
    *,
    ...
    from .emitters import _edge_delays_any_positive

    _validate_edge_delays_nonnegative_eager(edges)
    if _edge_delays_any_positive(edges):
        raise ValueError(
            "registered HDP rules currently require zero edge delays"
        )
