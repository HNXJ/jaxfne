...
def _build_atlas(slug: str, title: str, model, signals, **manifest_kw):
    ...
    manifest = build_atlas(model, signals, out_dir=str(out_dir), title=title,
                           **manifest_kw)
...
def spec_single_neuron():
    ...
    return ("Single neuron (1n)", model, signals,
            dict(duration_ms=100.0, dt_ms=0.1, seed=0))
...
def spec_two_neuron_ei():
    ...
    signals = model.simulate(jtfne.simulation(duration_ms=500.0, dt_ms=0.5,
                                              seed=0))
    return ("Two-neuron E/I (2n)", model, signals,
            dict(duration_ms=500.0, dt_ms=0.5, seed=0))


def spec_network_100_ei():
    import jaxfne as jtfne
    cfg = (jtfne.configuration()
           .network(name="network_100_ei", kind="balanced_ei_population",
                    n=100, cell_types={"E": 0.75, "PV": 0.25})
           .emitter(family="izhikevich", preset="cortical_eig")
           .field(domain="laminar_column", conductivity="proxy",
                  boundary="declared_proxy", gauge="mean_zero")
           .probe(name="multimodal_100_ei",
                  modes=["spikes", "V_m", "source", "phi_e", "J_e", "CSD",
                         "LFP"]))
    model = jtfne.construct(cfg)
    signals = model.simulate(
        jtfne.simulation(duration_ms=100.0, dt_ms=0.1, seed=42))
    return ("Balanced E/I population (100n)", model, signals,
...
def spec_v1_pfc_dual():
    ...
    from scripts.v1_pfc_continuous_aaab_smoke_test import (
        build_tensor, build_trial_schedule, tuning_group_indices)
    tensor = build_tensor()
    model = jtfne.construct_neuronal_tensor(tensor, seed=0,
                                            duration_ms=1000.0, dt_ms=0.1)
    groups = tuning_group_indices(model)
    schedule = build_trial_schedule(len(model.neuron_table()), groups)
    signals = jtfne.simulate(model, duration_ms=1000.0, dt_ms=0.5, seed=0,
                             paradigm=schedule)
    return ("V1-PFC dual column, single AAAB trial (200n, no HDP carryover)",
            model, signals, dict(duration_ms=1000.0, dt_ms=0.5, seed=0))
...
def spec_ei_population_100():
    ...
    return ("Chainable E/I population (100n)", model, signals,
...
def spec_suite1_column():
    ...
    cfg = (jtfne.Configuration()
           ...
           .column(name="laminar_column", layers=["L2/3", "L4", "L5", "L6"],
                   n=48)
    ...
    return ("Suite No. 1 laminar column, smoke-scale (48n, 1000 ms of 5000 ms)",
            model, signals, dict(duration_ms=1000.0, dt_ms=0.5, seed=44))


def spec_suite2_net1():
    import jaxfne as jtfne
    cfg = jtfne.suite2_net1_config(seed=7, n=100, duration_ms=1000.0,
                                   dt_ms=0.5)
    model = jtfne.construct(cfg)
    bundle = jtfne.suite2_run_bundle(model, seed=7, duration_ms=1000.0,
                                     dt_ms=0.5)
    return ("Suite No. 2 net1 (100n)", model, bundle["signals"],
...
def spec_evoked_l4():
    ...
    paradigm = jtfne.evoked_l4_drive_paradigm(
        l4_onset_ms=500.0, l4_duration_ms=200.0, l4_amplitude=1.0,
        pre_stimulus_buffer_ms=200.0, post_stimulus_buffer_ms=500.0)
    model = jtfne.construct(cfg)
    signals = jtfne.simulate(model, seed=7, duration_ms=1500.0, dt_ms=0.5,
                             paradigm=paradigm)
    return ("Evoked L4 drive, evoked condition (100n)", model, signals,
...
def spec_scale_100():
    ...
    model.params["emitter"] = dataclasses.replace(
        emitter_params, drive=new_drive, v0=new_v0,
        u0=emitter_params.b * new_v0, W=emitter_params.W * 0.05)
    signals = jtfne.simulate(model, duration_ms=1000.0, dt_ms=0.5, seed=seed)
    return ("Suite No. 3 scale N=100 (async patch)", model, signals,
...
def spec_source_column_48():
    ...
    cfg = (jtfne.Configuration()
           ...
           .column(name="tutorial_column", layers=["L2/3", "L4", "L5", "L6"],
                   n=48)
    ...
    model = jtfne.construct(cfg)
    signals = jtfne.simulate(model, duration_ms=1000.0, dt_ms=0.5, seed=42)
    return ("Source-bookkeeping column (48n)", model, signals,
            dict(duration_ms=1000.0, dt_ms=0.5, seed=42))
...
def spec_lfp_csd_12():
    ...
    cfg = (jtfne.Configuration()
           .runtime(seed=42, dtype="float32", duration_ms=1000.0, dt_ms=0.5)
           .column(name="laminar_lfp_csd", layers=["L2/3", "L4", "L5", "L6"],
                   n=12)
    ...
    model = jtfne.construct(cfg)
    signals = jtfne.simulate(model, duration_ms=1000.0, dt_ms=0.5, seed=42)
    return ("LFP/CSD laminar column (12n)", model, signals,
            dict(duration_ms=1000.0, dt_ms=0.5, seed=42))


def spec_eeg_meg_emm_100():
    import jaxfne as jtfne
    cfg = (jtfne.Configuration()
           .runtime(seed=42, dtype="float32", duration_ms=1000.0, dt_ms=0.5)
           .column(name="L2/3_column", layers=["L2/3"], n=100)
    ...
    model = jtfne.construct(cfg)
    signals = jtfne.simulate(model, duration_ms=1000.0, dt_ms=0.5, seed=42)
    return ("EEG/MEG/EMM proxy bundle (100n)", model, signals,
            dict(duration_ms=1000.0, dt_ms=0.5, seed=42))
...
def spec_omission_60():
    ...
    signals = jtfne.simulate(model, duration_ms=1000.0, dt_ms=0.5, seed=42)
    return ("Omission-oddball column, plain drive (60n)", model, signals,
            dict(duration_ms=1000.0, dt_ms=0.5, seed=42))


def spec_v1v4_80():
    import jaxfne as jtfne
    cfg = jtfne.suite2_v1_v4_config(seed=42, n_per_area=80,
                                    duration_ms=1000.0, dt_ms=0.5)
    model = jtfne.construct(cfg)
    signals = jtfne.simulate(model, duration_ms=1000.0, dt_ms=0.5, seed=42)
    return ("V1-V4 scaffold (80/area)", model, signals,
            dict(duration_ms=1000.0, dt_ms=0.5, seed=42))
...
def spec_canonical_etude_1000():
    ...
    cfg = (jtfne.laminar_cortex_config(
        ...
        baseline_drive_by_cell_type={"E": 5.0, "PV": 5.0, "SST": 5.0,
                                     "VIP": 5.0})
        .layer_fractions(layer_fractions=zbands)
        .area_layer_cell_types("V1", fractions))
    model = jtfne.construct(cfg)
    signals = jtfne.simulate(
        model, jtfne.simulation(duration_ms=1000.0, dt_ms=0.5, seed=0))
    return ("Canonical-column etude, uniform drive (1000n)", model, signals,
            dict(duration_ms=1000.0, dt_ms=0.5, seed=0))
...
def spec_hdp_1000():
    ...
    import jaxfne as jtfne
    jtfne.enable_x64()
    cfg = (jtfne.build_laminar_column(n=1000)
           .set_emitter("izhikevich", "cortical_eig")
           .probes(["spikes", "V_m"])
           .field(domain="laminar_column", conductivity="proxy")
           .runtime(duration_ms=200.0, dt_ms=0.5, seed=0, enable_hdp=True,
                    hdp_params={"K_HDP": 0.01, "tau_0_ms": 200.0,
                                "K_ctrl": 5.0, "barrier_c": 0.01,
                                "barrier_d": 0.01,
                                "record_weight_trace": False}))
    model = jtfne.construct(cfg)
    signals = jtfne.simulate(model)
    assert model.last_hdp_diagnostics() is not None
    return ("HDP column, short run (1000n, 200 ms)", model, signals,
...
def spec_homeostasis_1000():
    ...
    cfg = (jtfne.build_laminar_column(n=1000)
           ...
           .probes(["spikes", "V_m"])
           .field(domain="laminar_column", conductivity="proxy")
           .runtime(duration_ms=1000.0, dt_ms=0.5, seed=0,
                    enable_homeostasis=True,
                    homeostasis_params={"k_gain": 1.0, "r_star": 0.01}))
    model = jtfne.construct(cfg)
    signals = jtfne.simulate(model)
    return ("Homeostasis column (1000n)", model, signals,
            dict(duration_ms=1000.0, dt_ms=0.5, seed=0))
...
def spec_calibration_100():
    ...
    return ("Calibration-ready column, minimal run (100n, 200 ms)", model,
            signals, dict(duration_ms=200.0, dt_ms=0.5, seed=0))
...
def spec_objective_60():
    ...
    model = jtfne.construct(cfg)
    signals = jtfne.simulate(model, duration_ms=200.0, dt_ms=0.5, seed=1,
                             paradigm=paradigm)
    return ("Objective-grammar chain, pre-tune (60n)", model, signals,
...
def spec_operator_chain_40():
    ...
    cfg = jtfne.laminar_cortex_config(
        ...
        layers=("L1", "L4", "L6"),
        cell_types={"E": 0.6, "PV": 0.25, "SST": 0.15}, n=40,
        emitter="izhikevich")
    model = jtfne.construct(cfg)
    signals = jtfne.simulate(model, duration_ms=100.0, dt_ms=0.5, seed=3)
    return ("Operator-composition column (40n)", model, signals,
...
def spec_probe_32():
    ...
    model = jtfne.construct(cfg)
    signals = jtfne.simulate(model, duration_ms=200.0, dt_ms=0.5, seed=0)
    return ("Probe-operators circuit, minimal run (32n, 200 ms)", model,
            signals, dict(duration_ms=200.0, dt_ms=0.5, seed=0))
...
def spec_config_grammar_1000():
    ...
    cfg = (jtfne.Configuration()
           ...
           .geometry(layer_thickness={"L1": 0.1, "L2/3": 0.3, "L4": 0.2,
                                      "L5": 0.3, "L6": 0.3})
    ...
    model = jtfne.construct(cfg)
    signals = jtfne.simulate(model, duration_ms=200.0, dt_ms=0.5, seed=0)
    return ("Configuration-grammar column, smoke-scale (1000n, 200 ms)",
            model, signals, dict(duration_ms=200.0, dt_ms=0.5, seed=0))
...
SPECS = {
    ...
    "eeg_meg_emm_100": spec_eeg_meg_emm_100,
...
def spec_jaxley_single():
    ...
    cell.insert(HH())
    cell.record("v")
    cell.stimulate(jx.step_current(i_delay=10, i_dur=50, i_amp=0.1,
                                   delta_t=0.025, t_max=100))
    sig = jtfne.JaxleyBridge(model=cell).simulate(duration_ms=100.0,
                                                  dt_ms=0.025)
    fig = C.plot_membrane_potentials(sig, backend="plotly",
                                     title="Jaxley HH single compartment — V_m proxy")
    ...
    out.write_text(fig.to_html(include_plotlyjs="cdn", full_html=True),
                   encoding="utf-8")
    print(f"[jaxley_single] wrote {out.relative_to(ROOT)} "
          f"({out.stat().st_size}B) V_m={tuple(sig.V_m.shape)}", flush=True)
...
def main(argv=None):
    ...
    parser.add_argument("--slug", default="",
                        help="comma-separated subset (plus 'jaxley_single')")
    ...
    wanted = ([s.strip() for s in args.slug.split(",") if s.strip()]
              if args.slug else [s for s in names if s != "jaxley_single"])
