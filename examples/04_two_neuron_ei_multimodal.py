#!/usr/bin/env python3
"""
Two-neuron E/I multimodal proxy probe tutorial.

Demonstrates the v0.2.1 multimodal proxy probe stack (eight operators)
on a minimal two-neuron excitatory/inhibitory network.
...
Status metadata: All operators are simulated proxies with frozen
validation metadata. Simulated mechanism-readout outputs. No empirical validation.

Usage:
    python examples/04_two_neuron_ei_multimodal.py

Generates:
    outputs/v029_two_neuron_ei_multimodal/
...
def main():
    # === 1. Create two-neuron E/I configuration ===
    ...
    # === 8. Status metadata verification ===
    validation_report = {
    ...
    # === 9. Write outputs ===
    output_dir = pathlib.Path("outputs/v029_two_neuron_ei_multimodal")
    ...
    try:
        ...
        # Spike raster for 2-neuron network
        ...
        for neuron_idx in range(spikes.shape[0]):
            ...
            color = 'blue' if neuron_idx == 0 else 'red'  # E: blue, I: red
            ax.vlines(spike_times, neuron_idx - 0.4, neuron_idx + 0.4, colors=color, linewidth=1.0, alpha=0.8)

        ax.set_xlabel("Timestep")
        ax.set_ylabel("Neuron (E: blue, I: red)")
        ax.set_title("Two-neuron E/I spike trains")
    ...
    source_data = {
        ...
        "tutorial_id": "04_two_neuron_ei_multimodal",
        "figure_id": "raster",
        "time_ms": spike_times,
        "unit_id": unit_ids,
    ...
    print("=== Two-neuron E/I Multimodal Proxy Tutorial ===\n")
