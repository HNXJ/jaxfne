#!/usr/bin/env python3
"""Programmatically build the Suite No. 2 Jupyter notebook."""

import json
import pathlib

def main():
    notebook = {
        "cells": [
            {
                "cell_type": "markdown",
                "metadata": {},
                "source": [
                    "# jaxfne Suite No. 2: Spectrolaminar Motif\n",
                    "\n",
                    "This tutorial demonstrates the jaxfne Suite No. 2 pipeline. We declare a multi-column layout (V1 and PFC), configure laminar details, run a spontaneous activity simulation, and visualize the 3-panel spectrolaminar profile."
                ]
            },
            {
                "cell_type": "markdown",
                "metadata": {},
                "source": [
                    "## 1. Setup and imports"
                ]
            },
            {
                "cell_type": "code",
                "execution_count": None,
                "metadata": {},
                "outputs": [],
                "source": [
                    "import jaxfne as jtfne\n",
                    "import matplotlib.pyplot as plt\n",
                    "import numpy as np"
                ]
            },
            {
                "cell_type": "markdown",
                "metadata": {},
                "source": [
                    "## 2. Declare Chainable Configuration"
                ]
            },
            {
                "cell_type": "code",
                "execution_count": None,
                "metadata": {},
                "outputs": [],
                "source": [
                    "cfg = jtfne.Configuration()\n",
                    "cfg = cfg.set_runtime(seed=7, dtype=\"float32\", duration_ms=1000.0, dt_ms=0.1)\n",
                    "cfg = cfg.add_column(\"V1\", layers=[\"L2/3\", \"L4\", \"L5\", \"L6\"], n=80)\n",
                    "cfg = cfg.add_column(\"PFC\", layers=[\"L2/3\", \"L5\", \"L6\"], n=80)\n",
                    "cfg = cfg.set_cell_types({\"E\": 0.75, \"PV\": 0.12, \"SST\": 0.08, \"VIP\": 0.05})\n",
                    "cfg = cfg.set_connectivity(feedforward=(\"V1\", \"PFC\"), feedback=(\"PFC\", \"V1\"))\n",
                    "cfg = cfg.set_emitter(\"izhikevich\", \"cortical_eig\")\n",
                    "cfg = cfg.set_probes([\"MUA-proxy\", \"LFP-proxy\", \"CSD-proxy\", \"EEG-proxy\", \"MEG-proxy\", \"EMM-proxy\"])"
                ]
            },
            {
                "cell_type": "markdown",
                "metadata": {},
                "source": [
                    "## 3. Construct V1/PFC model"
                ]
            },
            {
                "cell_type": "code",
                "execution_count": None,
                "metadata": {},
                "outputs": [],
                "source": [
                    "model = jtfne.construct(cfg)"
                ]
            },
            {
                "cell_type": "markdown",
                "metadata": {},
                "source": [
                    "## 4. Run the simulation"
                ]
            },
            {
                "cell_type": "code",
                "execution_count": None,
                "metadata": {},
                "outputs": [],
                "source": [
                    "signals = jtfne.simulate(model, duration_ms=1000.0, dt_ms=0.1, seed=7)"
                ]
            },
            {
                "cell_type": "markdown",
                "metadata": {},
                "source": [
                    "## 5. Spectrolaminar Motif Visualization"
                ]
            },
            {
                "cell_type": "code",
                "execution_count": None,
                "metadata": {},
                "outputs": [],
                "source": [
                    "fig = jtfne.vis.spectrolaminar(signals)\n",
                    "plt.show()"
                ]
            }
        ],
        "metadata": {
            "kernelspec": {
                "display_name": "Python 3",
                "language": "python",
                "name": "python3"
            },
            "language_info": {
                "name": "python"
            }
        },
        "nbformat": 4,
        "nbformat_minor": 2
    }

    target = pathlib.Path("tutorials/jaxfne_suite_no_2_spectrolaminar_motif.ipynb")
    target.parent.mkdir(parents=True, exist_ok=True)
    with open(target, "w") as f:
        json.dump(notebook, f, indent=2)

    print("build_notebook=PASS")

if __name__ == "__main__":
    main()
