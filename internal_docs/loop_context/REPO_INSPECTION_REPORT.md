...
missing publication figures: ed09_failure_modes_and_nulls.png; ed10_release_archive_receipt.png
outputs/publication in zip: missing
```
...
python3 -m json.tool outputs/publication/inventory.json >/dev/null
python3 -m compileall -q jaxfne scripts/publication
...
| Check | Result |
|---|---|
| `publication_inventory.py` | passed; generated local `outputs/publication/inventory.json`; reported 8/8 main and 8/10 ED |
| `publication_checklist.json` strict JSON | passed |
| generated `inventory.json` strict JSON | passed |
| `compileall jaxfne scripts/publication` | passed |
...
3. The attached zip contains figure PNGs and generator scripts but not committed `outputs/publication/*` manifests/receipts expected by the checklist.
